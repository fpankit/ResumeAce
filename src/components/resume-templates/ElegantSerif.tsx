  ...
  return (
    <div className="space-y-12" style={s}>
      <header className="text-center border-b-2 border-double pb-10" style={{ borderColor: theme.primary, ...spacing }}>
        ...
        <div className="flex justify-center gap-6 text-[10px] font-bold opacity-40 uppercase tracking-widest mt-4">
      ...
      {sections.experience && data.experience.length > 0 && (
        <section className="space-y-10" style={spacing}>
          <h2 className="text-center text-xs font-black uppercase tracking-[0.4em] opacity-30">Experience</h2>
          {data.experience.map(exp => (
            <div key={exp.id} className="text-center space-y-2">
              <h3 className="text-xl font-bold text-slate-900">{exp.title}</h3>
              <p className="text-[10px] font-black uppercase" style={{ color: theme.accent }}>
                {exp.company} | {exp.startMonth} {exp.startYear} – {exp.current ? 'Present' : `${exp.endMonth} ${exp.endYear}`}
              </p>
              <div className="text-slate-600 max-w-2xl mx-auto text-sm leading-relaxed whitespace-pre-wrap">
